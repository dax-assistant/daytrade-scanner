...
class ConfigAndPolicyTests(unittest.TestCase):
    ...
    def test_policy_uses_live_notional_cap(self):
        ...
        auto_blocked = policy.evaluate_order_intent(OrderIntent(symbol="AMD", side="buy", qty=1, estimated_price=10, estimated_notional=100, source="scanner_auto"))
        allowed = policy.evaluate_order_intent(OrderIntent(symbol="AMD", side="buy", qty=1, estimated_price=10, estimated_notional=100, source="manual_entry"))
        blocked = policy.evaluate_order_intent(OrderIntent(symbol="AMD", side="buy", qty=100, estimated_price=10, estimated_notional=1000, source="manual_entry"))

        self.assertFalse(auto_blocked.allowed)
        self.assertEqual(auto_blocked.reason_code, "manual_live_entry_required")
